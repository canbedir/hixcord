      ...
      <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-neutral-400 space-y-[2px]">
        {isModerator && (
          <DropdownMenuItem onClick={()=> onOpen("invite",{ server })} 
           className="text-indigo-600 dark:text-indigo-400 px-3 py-2 text-sm cursor-pointer focus:bg-indigo-500 dark:hover:text-white"
          >
            Invite People
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={()=>onOpen("deleteServer",{server})}
           className="px-3 py-2 text-sm cursor-pointer focus:text-white text-red-600 focus:bg-red-500 dark:hover:text-white">
            Delete Server
